...
### Thu 09 Jul 2020 02:32:21 PM CST
...
[Porting Any Custom Roms fastest method](https://forum.xda-developers.com/t/guide-porting-any-custom-roms-fastest-method-update-for-porting-miui-kitkat.2769142/)
一个替换rom文件的帖子给了一点启发，基他的保持不变， 只留下通用的系统启动部分， miui 的 init.miui.rc 保证里面的程序都OK，一个系统定制就完成了。 
实际上也应该是这样，定制系统也不过是用脚本的形式来替换, 然后就不用每次对rom修改部分做同样操作。 

系统定制的思想应该很明确的，系统应用与底层的驱动是解耦的，使用hidl来通信， 复用原ROM的驱动部分，但是肯定有一些额外的定制功能就没办法使用， 删掉它， 只要不影响系统正常运行就OK。 

哪一个程序少了， 哪一个库少了，补哪个就是了， 脚本里的那几个变量， 也仅仅是定制了一套复制规则。 从哪里查呢？ 
...
system/framework/res/storage-list.xml

上面的文件使用原ROM, 其他的内容可替换。 
